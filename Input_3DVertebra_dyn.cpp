...
namespace Input
{
                 ...
                 ArrayXPI &Nel, double &R, Eigen::VectorXd &zIni)
  {
      ...
      topOpt->pmin = 1.0; topOpt->pstep = 1.0; topOpt->pmax = 3.0;

      optmma->Set_KKT_Limit(0);
      optmma->Set_Iter_Limit_Min(5);
      optmma->Set_Iter_Limit_Max(15);
              ...
              Eigen::VectorXd &g, Eigen::MatrixXd &dgdx)
  {
    ...
    PetscInt nevals = 10, noptev = 2;
    double factor = 1.0/9.0;

    f = 0;
    dfdx.setZero(topOpt->nLocElem);
    g.setZero(nevals-noptev+1);
    dgdx.setZero(topOpt->nLocElem,nevals-noptev+1);

    double temp;
    Eigen::MatrixXd dtemp = Eigen::VectorXd::Zero(topOpt->nLocElem, 1);
    ierr = Functions::Compliance( topOpt, temp, dtemp.data() ); CHKERRQ(ierr);
    double WC = 0.05;
    f += WC*temp;
    dfdx += WC*dtemp;
    ierr = Functions::Perimeter( topOpt, temp, dtemp.data() ); CHKERRQ(ierr);
    double WP = 0.35;
    f -= WP*temp;
    dfdx -= WP*dtemp;
    ...
    ierr = PetscOptionsGetInt(NULL,NULL,"-nev",&nevals,NULL); CHKERRQ(ierr);
    Eigen::VectorXd tempvec = Eigen::VectorXd::Zero(nevals);
    dtemp.setZero(topOpt->nLocElem,nevals);
    ierr = Functions::Dynamic( topOpt, tempvec.data(), dtemp.data(), nevals ); CHKERRQ(ierr);
    // Weight the results
    double WD = 0.60;
    tempvec.segment(0, nevals) *= WD; dtemp.block(0, 0, topOpt->nLocElem, nevals) *= WD;
    // Combine all the frequencies that make up the objective
    Eigen::VectorXd Adjustment = Eigen::VectorXd::LinSpaced(noptev,noptev-1,0)*(1-factor)/(1-pow(factor,noptev));
    f += Adjustment.dot(tempvec.segment(0,noptev));
    dfdx += dtemp.block(0, 0, dtemp.rows(), noptev)*Adjustment;
    // Add the rest as constraints
    for (short i = noptev; i < nevals; i++)
        g(i-noptev+1) = tempvec(i) - pow(0.99,i-1)*tempvec(noptev-1);
    dgdx.block(0, 1, topOpt->nLocElem, nevals-noptev) = dtemp.block(0, noptev, topOpt->nLocElem, nevals-noptev);
    //f += 0.2*g(0); dfdx += 0.2*dgdx.col(0);


    /// Volume Subproblem
    ierr = Functions::Volume( topOpt, g(0), dgdx.data() ); CHKERRQ(ierr);
    g(0) -= 0.2;
