...
namespace Functions
{
  ...
  int Buckling( TopOpt *topOpt, double *lambda, double *grad, PetscInt &nevals )
  {
    ...
    for (short i = 0; i < nev_conv; i++)
    {
      ...
      ierr = PetscPrintf(topOpt->comm, "Solve for adjoint equation #%i converged in %i iterations with reason: %i\n", i, its, reason); CHKERRQ(ierr);
    ...
    if (nev_conv < nevals)
    {
      PetscPrintf(topOpt->comm, "***************************************************\n");
      PetscPrintf(topOpt->comm, "Warning, nevals < nev_conv\n");
      PetscPrintf(topOpt->comm, "***************************************************\n");
    }
